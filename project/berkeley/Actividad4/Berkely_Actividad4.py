...
#Actividad 4 - Simular latencia de red con retrasos aleatorios en los clientes
...
def ClienteBerkeley(client_id, port=60001):
    ...
    print(f"    [CLIENTE {client_id:02d}] Variación de reloj: {variacion:+.4f} seg")
    print(f"    [CLIENTE {client_id:02d}] Tiempo local: {local_time:.6f}")

    data = sockClient.recv(1024)
    if data == b'TIME_REQUEST':
        # Simular latencia de red al enviar el tiempo
        latencia_envio = random.uniform(0, 3)
        print(f"    [CLIENTE {client_id:02d}] ~ Latencia de envío: {latencia_envio:.3f} seg")
    ...
    # Simular latencia de red al recibir el ajuste
    latencia_recepcion = random.uniform(0, 3)
    print(f"    [CLIENTE {client_id:02d}] ~ Latencia de recepción: {latencia_recepcion:.3f} seg")
    time.sleep(latencia_recepcion)

    adjustment = struct.unpack('f', sockClient.recv(4))[0]
    new_time = local_time + adjustment

    print(f"    [CLIENTE {client_id:02d}] ─────────────────────────────────")
    print(f"    [CLIENTE {client_id:02d}] Latencia total: {(latencia_envio + latencia_recepcion):.3f} seg")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("ALGORITMO DE BERKELEY - ACTIVIDAD 4")
    print("Simulación de latencia de red (0-3 segundos)")
    print("="*60)
