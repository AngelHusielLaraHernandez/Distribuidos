//Se usan 6 procesos y se empieza con una matriz de 10x10
//el proceso 0 envia a todos los procesos del 1 al 5, le va a enviar a cada procesos dos renglones y cada proceso lo vuelve a enviar a la matriz 
//Cada proceso tiene que tener un arreglo de 2x10

#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>


int main(int argc, char** argv) {
    int np,id,n=6;
    int matriz[10][10];
    int matrizaux[2][10];

    MPI_Status estado;
    MPI_Init(&argc, &argv); // inicializa el entorno
    MPI_Comm_size(MPI_COMM_WORLD, &np); // obtiene el número de procesos y se guarda la direccion en np
    MPI_Comm_rank(MPI_COMM_WORLD, &id); // obtiene el identificador del proceso y se guarda en id

    if(id==0){
        for(int i=0; i<10; i++){
            for(int j=0; j<10; j++){
                matriz[i][j]=i+1;
            }
        }

        for(int i=1; i<np; i++){
            MPI_Send(&matriz[(i-1)*2][0],20,MPI_INT,i,i,MPI_COMM_WORLD);
        }
    }
    else{
        MPI_Recv(&matrizaux[0][0],20,MPI_INT,0,id,MPI_COMM_WORLD,&estado);
        for(int i=0; i<2; i++){
            for(int j=0; j<10; j++){
                printf("Proceso %d recibio el dato %d\n", id, matrizaux[i][j]);
            }
        }
    }

    MPI_Finalize();
    return 0;
}